""" safer and more sabatized code than app.py """

from typing import List, Optional, Dict, Any
from datetime import datetime
import re, os, tempfile, uuid
...
engine = create_engine("sqlite:///./ats.db", connect_args={"check_same_thread": False})
SQLModel.metadata.create_all(engine)

app = FastAPI(title="ATS", version="1.0")
...
def score_keywords(resume_text: str, required_kw: List[str], nice_kw: List[str]) -> Dict[str, Any]:
    ...
    bonus = 2 if required_kw and len(req_hits) == len(required_kw) else 0
    ...
    return {
        ...
        "missing_required": [k for k in required_kw if k not in req_hits],
        "bonus_all_required": bool(bonus),
    }

def semantic_similarity(a: str, b: str) -> float:
    ea = embedder.encode(sanitize_text(a), convert_to_tensor=True)
    eb = embedder.encode(sanitize_text(b), convert_to_tensor=True)
...
def create_application(payload: ApplicationCreate):
    with Session(engine) as session:
        ...
        appn = Application(job_id=job.id, candidate_id=cand.id, resume_text=sanitize_text(payload.resume_text), score=final)
        session.add(appn)
        session.commit()
...
async def upload_resume(job_id: int, candidate_id: int, file: UploadFile = File(...)):
    ...
    with Session(engine) as session:
        ...
        appn = Application(job_id=job.id, candidate_id=cand.id, resume_text=sanitize_text(text), score=final)
        session.add(appn)
        session.commit()
        session.refresh(appn)
